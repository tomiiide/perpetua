...
import { decIsNegative, decToNumber } from "../decimal/index.js";
import type { FormattedParts } from "./types.js";

/** e.g. 1.2M, 340K. Explicitly lossy (CORE_SPEC.md §3) — for compact display only. */
export function formatCompact(v: Dec, locale = "en-US"): FormattedParts {
  const n = decToNumber(v);
  const text = new Intl.NumberFormat(locale, { notation: "compact", maximumFractionDigits: 1 }).format(n);
  const negative = decIsNegative(v);
